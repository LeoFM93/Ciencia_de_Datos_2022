...
* Diagnosticamos la magnitud de los valores perdidos
...
* Observamos:
  ...
  + Que nuestras variables para los Códigos de Región y los Códigos de Provincia son de clase 'integer' y, gracias a la visualización que nos retorna ***View()***, no considera los '0' a la izquierda, lo que genera problemas con la graficación más adelante.
...
* Entonces:
  ...
  + Completamos los valores de 'COD_REGION' y 'COD_PROVINCIA' con '0' hasta completar las 06 cifras.
...
bd_reactiva_pyto_reg <- bd_reactiva_pyto %>% 
  ...
  select('COD_REGION', 'REGION', 'TOTALEMPLEOS', 'MONTODEINVERSIÓN', 'MONTODETRANSFERENCIA2020', 'POBLACIONBENEFICIARIA', 'UBIGEO', 'TIPOLOGIA') 
...
bd_reactiva_pyto_prov <- bd_reactiva_pyto %>% 
  ...
  mutate(MONTODETRANSFERENCIA2020 = str_remove_all(MONTODETRANSFERENCIA2020, ","),
         ...
         MONTODEINVERSIÓN = as.numeric(MONTODEINVERSIÓN)) %>%
  filter(row_number() != 526) %>%
  select('COD_PROVINCIA', 'PROVINCIA', 'TOTALEMPLEOS', 'MONTODEINVERSIÓN', 'MONTODETRANSFERENCIA2020', 'POBLACIONBENEFICIARIA', 'UBIGEO') 
...
* Entonces:
  ...
  + Agregamos fila con el código correspondiente a la región faltante, Madre de Dios, y asignamos valor '0' para la variable de Monto de Transferencia.
  
  + Agregamos filas con el código correspondiente a cada provincia faltante y asignamos valor '0' para la variable de Monto de Transferencia.

```{r eval=TRUE, echo=TRUE}

# Creamos fila de región faltante
new_file_reg = data.frame( COD_REGION = "-", 
...
#### 3.2.1. Distribución de Montos Transferidos por Región Y Provincia

<br>

DISTRIBUCIÓN DE LOS RECURSOS TRANSFERIDOS (PROCESAMIENTO): 
...
#### 3.2.2. Porcentaje del Costo del Proyecto, o Monto de Inversión, cubierto por el Monto Transferidos

<br>

#### 3.2.3. Relación entre Población Beneficiada por Proyectos Concluidos y la Población Total

<br>

#### 3.2.4. Relación entre Empleos Generados, Monto de Inversión y Monto de Transferencia

<br>

#### 3.2.5. Monto de inversión por tipo de intervención

<br>

### 3.3.  Técnica y Algoritmos aplicados: 
...
#### 5.4. Relación entre Empleos Generados, Monto de Inversión y Monto de Transferencia:

REDACTAR INTERPRETACIÓN

> Generamos 01 data frame con nuestras 08 variables de interés.

* Para esto:
1. Completamos los valores de 'COD_REGION' y 'COD_PROVINCIA' con '0' hasta completar las 6 cifras. Al ser importado como 'character', no considera los ceros a la izquierda, lo que genera problemas con la graficación más adelante.
2. Convertimos de 'character' a 'numeric' la clase de 'MONTOTRANSFERENCIA2020' y 'MONTODEINVERSIÓN'. Removemos la coma separador de miles para que reconozca nuestros valores como número y no texto.
...
* Examinamos nuestro objeto y sus variables.
...
> Generamos mapas de la relación entre Total de Empleos, Monto de Inversión y Monto Transferido.
...
* Nuebamente, desde la base de datos "bd_reactiva_pyto_reg" y la librería ggplot obtenemos un gráfico de barras que esta vez muestra los montos transferidos por tipo de proyecto.
